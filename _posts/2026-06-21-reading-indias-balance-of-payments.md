---
layout: post
title: "Reading India's Balance of Payments Without the Panic"
dek: "A live data class on the rupee, the trade deficit, and why comparing today to 1991 is lazy history — using the balance of payments table instead of the headline dollar rate."
date: 2026-06-21
session: 15
tags: [balance-of-payments, forex-reserves, rupee, india, applied-macro]
excerpt_override: "Session 15 — Tantri walks the actual BoP table, row by row, to show why the rupee's fall is a choice RBI is making, not a crisis it's suffering."
authors:
  - harsh
editors: []
---
...
<div class="widget" id="w-bop">
  <p class="w-head">RESERVE DEFENSE, AT A COST</p>
  <p class="w-note" style="margin:0 0 .6rem">Move the rupee level RBI could choose to defend, and see roughly how much of the ~$700bn reserve buffer that costs — using the ballpark Tantri gives in class (₹90–87 costing $200–300bn).</p>
  <div class="w-row"><label>Rupee/USD level RBI defends</label><input type="range" data-x min="85" max="98" step="1" value="97"><output data-x-label></output></div>
  <p class="w-big" data-out></p>
  <p class="w-note" data-verdict></p>
</div>
<script>
(function(){
  var root = document.getElementById("w-bop"); if (!root) return;
  var x = root.querySelector("[data-x]");
  var out = root.querySelector("[data-out]"), verdict = root.querySelector("[data-verdict]");
  function draw(){
    var v = +x.value;
    root.querySelector("[data-x-label]").textContent = "₹" + v;
    // Rough linear interpolation anchored on his own numbers in class:
    // ~97 (current, ~status quo) costs ~$0bn extra; ~90-87 costs $200-300bn.
    var gap = 97 - v;
    var costLow = Math.round(gap * (200/9));
    var costHigh = Math.round(gap * (300/9));
    if (gap <= 0) {
      out.textContent = "$0bn of extra reserves spent";
      verdict.textContent = "This is roughly where RBI is choosing to let the rupee sit right now — no extra reserve defense needed.";
    } else {
      out.textContent = "~$" + costLow + "–" + costHigh + "bn of reserves spent defending it";
      verdict.textContent = "Reserves are ~$700bn, so this is affordable — which is exactly Tantri's point: the weaker rupee is a choice, not a helpless slide, because RBI could pay this to pull it back and isn't.";
    }
  }
...
<div class="widget mcq" data-mcq data-answer="b">
  ...
  <p class="mcq-q">India's goods deficit last year was about $337 billion, but Tantri insists that "goods deficit" and "trade deficit" are not the same thing, and that newspapers routinely conflate them. What closes most of that gap?</p>
  <div class="mcq-opts">
    <button type="button" data-opt="a">A one-time RBI gold sale</button>
    <button type="button" data-opt="b">A $216 billion services surplus, which is still accelerating</button>
    <button type="button" data-opt="c">Falling oil prices reducing the import bill to near zero</button>
    <button type="button" data-opt="d">A large capital account surplus from FDI</button>
  </div>
  <p class="w-note" data-explain hidden>Services exports — not oil, not gold, not capital inflows — are what turn a $337bn goods deficit into a much smaller ~$121bn trade deficit (about 3.5% of GDP). Tantri's point: the newspaper number ("trade deficit") is usually just the goods number, because services data reports with a ten-day lag and reporters can't wait for it.</p>
</div>
<script>
(function(){
