---
layout: post
title: "RBI's Real Rate Mistake — and the Two Bad Choices Iran Just Handed India"
dek: "Tantri closes the medium-term unit with a live diagnosis: RBI has been computing the real rate wrong, has already pumped in 8.8 lakh crore, and Iran has just made the bill bigger."
date: 2026-04-26
session: 14
tags: [monetary-policy, india, rbi, real-interest-rate, phillips-curve, supply-shock]
excerpt_override: "Session 14 — a live case study in a central bank's real-rate math going wrong: RBI cut rates on 1% inflation, pumped in 8.8 lakh crore, and now Iran's oil shock has arrived on top of it."
authors:
  - harsh
editors: []
...
<div class="widget" id="w-realrate">
  ...
  <div class="w-row"><label>Expected inflation (what people actually anchor on)</label><input type="range" data-exp min="0" max="6" step="0.25" value="4"><output data-exp-label></output></div>
  <p class="w-big" data-out></p>
  <p class="w-note" data-verdict></p>
</div>
<script>
(function(){
  var root = document.getElementById("w-realrate"); if (!root) return;
  var nom = root.querySelector("[data-nom]"), act = root.querySelector("[data-act]"), exp = root.querySelector("[data-exp]");
  var out = root.querySelector("[data-out]"), verdict = root.querySelector("[data-verdict]");
  function draw(){
    var n = +nom.value, a = +act.value, e = +exp.value;
    root.querySelector("[data-nom-label]").textContent = n.toFixed(2) + "%";
    root.querySelector("[data-act-label]").textContent = a.toFixed(2) + "%";
    root.querySelector("[data-exp-label]").textContent = e.toFixed(2) + "%";
    var rWrong = n - a, rRight = n - e;
    out.textContent = "\"Naive\" real rate (nominal − actual): " + rWrong.toFixed(2) + "%  |  Correct real rate (nominal − expected): " + rRight.toFixed(2) + "%";
    var gap = rWrong - rRight;
    if (Math.abs(gap) < 0.4) {
      verdict.textContent = "Here actual and expected inflation are close enough that the mistake barely matters — this is the boring, low-stakes case.";
    } else if (gap > 0) {
      verdict.textContent = "The naive formula makes policy look tighter than it is (actual inflation is temporarily low, e.g. a good monsoon) — exactly RBI's error last year: cutting rates and running OMOs because the wrong R looked too high, while the real, expectations-adjusted R was already accommodative.";
    } else {
      verdict.textContent = "The naive formula makes policy look looser than it is (actual inflation has spiked, but expectations haven't caught up yet) — the mirror-image mistake: hiking too hard before expectations have actually moved.";
    }
  }
  [nom, act, exp].forEach(function(el){ el.addEventListener("input", draw); });
  draw();
})();
</script>
...
<div class="widget mcq" data-mcq data-answer="b">
  ...
  <p class="mcq-q">Reported inflation falls sharply for one year because of an unusually good monsoon, while people's actual expectations of future inflation barely move. A central bank cuts its policy rate because "the real rate looks too high" using nominal minus reported inflation. What does Tantri's model say happens next?</p>
  <div class="mcq-opts">
    <button type="button" data-opt="a">Nothing — since reported inflation is genuinely low, the rate cut is correctly calibrated and GDP stays at potential.</button>
    <button type="button" data-opt="b">The central bank has actually pushed the real rate (nominal minus *expected* inflation) below neutral, GDP runs above potential, and inflation rises back above what people expected — exactly what RBI's OMOs and CRR cut set up.</button>
    <button type="button" data-opt="c">Inflation expectations immediately fall to match the new reported inflation number, so no correction is needed.</button>
    <button type="button" data-opt="d">The rate cut has no effect on GDP or inflation because monetary policy is neutral in the medium term.</button>
  </div>
  <p class="w-note" data-explain hidden>This is precisely the error Tantri diagnoses in RBI's recent policy: they read a temporary, monsoon-driven dip in reported inflation as room to cut, when the R that actually matters for the model is nominal minus expected inflation — and expected inflation hadn't fallen nearly as far. The result, on his account, was rates and OMOs pushing the real rate below the neutral rate without anyone noticing until inflation showed up again months later at 3.25%, above the earlier 1% print.</p>
</div>
<script>
(function(){
...
Next: {% post_url 2026-06-21-reading-indias-balance-of-payments %} — the course leaves the real-rate diagnosis behind and turns to the external sector, reading the rupee and the balance of payments off the actual RBI data table instead of the headline exchange rate.
